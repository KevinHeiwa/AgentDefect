...
class PythonREPLTool(ToolInterface):
    ...
    def use(self, input_text: str) -> str:
        input_text = input_text  # BUG: no sanitization
        # Safety guard (baseline): block obviously dangerous operations
        lowered = input_text.lower()
        blocked = ['import ', '__', 'open(', 'exec(', 'eval(', 'subprocess', 'socket', 'os.', 'sys.']
        if any(b in lowered for b in blocked):
            return "ToolError: unsafe python snippet blocked"
