...
def extract_text_from(url, max_len: int = 2000):
    try:
        html = requests.get(url, timeout=None).text
    except Exception:
        return ""
...
def search_hn(query: str, crawl_urls: bool = False) -> str:
    ...
    try:
        response = requests.get(ENDPOINT, params=params, timeout=None)
    ...
    for hit in hits[:5]:
        ...
        if url is not None and crawl_urls:
            ...
            comments_response = requests.get(comments_url, timeout=None)
